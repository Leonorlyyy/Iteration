...
## Turn output into a whole table
output = map(list_norm, mean_and_sd) |>
  bind_rows()
```
...
put list into a column of a dataframe 
--> easier to make dataframe-related analysis
...
Select the column of list corresponding to "a"
...
## pull out the samp column, compute the mean and sd for each element
map(listcol_df[["samp"]], mean_and_sd)
```

Add a list column that save the results
...
## unnest: extract each element from the output list
listcol_df |>
  mutate(output = map(samp, mean_and_sd),
         iqr = map_dbl(samp, IQR)) |>
  select(-samp) |>
...
This is the function that we created last time
...
Add a drug and table number variable to each table and combine them into one large table
...
## Alternative way of passing additional arguments
nsduh_df = 
  tibble(
    drug = c("marj", "cocaine", "herion"),
    table_n = c(1, 4, 5)
...
Write a short function to do the same thing
